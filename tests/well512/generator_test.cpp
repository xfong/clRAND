...
int main(int argc, char **argv) {
    ...
    clrand_initialize_prng(test, (*tmpStructPtr).target_device, (*tmpStructPtr).ctx, CLRAND_GENERATOR_WELL512);
    ...
    well512_state* state_mem = new well512_state[numPRNGs];
    if (stateMemSize == numPRNGs * sizeof(well512_state)) {
        ...
        std::cout << "State Structure Size (host side): " << sizeof(well512_state) << std::endl;
    ...
    well512_state* golden_states = new well512_state[numPRNGs];
    ...
    for (int idx = 0; idx < numPRNGs; idx++) {
        ...
        well512_seed(&golden_states[idx], newSeed);
        if (golden_states[idx].i != state_mem[idx].i) {
            err_counts++;
            std::cout << "Mismatch in i at idx = " << idx << std::endl;
            continue;
        }
    ...
    for (int idx = 0; idx < numPRNGs; idx++) {
        hostRandomNumbers[idx] = well512_uint(golden_states[idx]);
